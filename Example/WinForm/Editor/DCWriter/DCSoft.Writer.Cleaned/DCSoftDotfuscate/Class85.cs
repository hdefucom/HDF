...
{
	internal class Class85 : Class84
	{
		...
		public Class85(string string_7)
		{
			string_0 = string_7;
		}
		...
		private string method_1()
		{
			int num = 14;
			string_1 = null;
			if (string_0.Length != 12 || !method_0(string_0))
			{
				string_1 = BarcodeStrings.UPCAInvaliData;
				return null;
			}
			string str = "101";
			string text = string_5[int.Parse(string_0[0].ToString())];
			str += string_2[int.Parse(string_0[0].ToString())];
			int i;
			for (i = 0; i < 5; i++)
			{
				if (text[i + 1] == 'a')
			...
			str += string_4[int.Parse(string_0[string_0.Length - 1].ToString())];
			str += "101";
			method_2();
			string key = "0" + string_0.Substring(0, 1);
			if (hashtable_0.ContainsKey(key))
			{
				string_6 = hashtable_0[key].ToString();
				return str;
			}
			string_1 = BarcodeStrings.UPCAInvaliCountry;
			return null;
		}
