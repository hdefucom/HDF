...
{
    /// <summary>
    /// <name>Emr文本文档页脚区域的文本块对象，继承自文本容器ZYTextContainer，为一个容器类型。</name>
    /// <auth>myc</auth>
    /// <date>2014-07-07</date>
    /// </summary>
    public class ZYDocumentFooter : ZYTextContainer
    {
        #region 基本属性或方法
        /// <summary>
        /// 构造函数，初始化此文本块对象，内部包含一个段落元素。
        /// </summary>
        public ZYDocumentFooter()
        {
            try
            {
        ...
        public override string ToString()
        {
            try
            {
                return "ZYDocumentFooter Name:" + myAttributes.GetString(ZYTextConst.c_Name)
        ...
        /// <returns></returns>
        public static ZYDocumentFooter GetOwnerDiv(ZYTextElement myElement)
        {
            try
            {
                ZYDocumentFooter myOwnerDiv = null;
                while (myElement != null)
                {
                    myOwnerDiv = myElement as ZYDocumentFooter;
                    if (myOwnerDiv == null)
                    {
