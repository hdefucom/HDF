...
namespace DCSoft.TemperatureChart
{
	...
	///       在WEB页面中显示文档内容的WEB控件
	///       </summary>
	[Designer(typeof(WebTemperatureControlDesigner))]
	[ToolboxBitmap(typeof(WebTemperatureControl))]
	[DCPublishAPI]
	[DocumentComment]
	[ControlBuilder(typeof(WebTemperatureControlBuilder))]
	[ComVisible(false)]
	public class WebTemperatureControl : WebControl
	{
		...
		private string string_7 = "DCTemperatureDocument";
		...
		public DocumentViewMode ViewMode
		{
			...
			set
			{
				...
				if (value == DocumentViewMode.Normal)
				{
					documentViewMode_0 = DocumentViewMode.Page;
				}
			}
		}
		...
		///       </summary>
		[Category("Data")]
		[DefaultValue("DCTemperatureDocument")]
		...
		public string RegisterCode
		{
			get
			{
				return TemperatureDocument.StaticRegisterCode;
			}
			set
			{
			}
		...
		private static bool smethod_0(HttpRequest httpRequest_0, HttpResponse httpResponse_0, HttpSessionState httpSessionState_0)
		{
			...
			{
				throw new ArgumentNullException("WebTemperatureControl.InnerHandleService.request");
			}
			if (httpResponse_0 == null)
			{
				throw new ArgumentNullException("WebTemperatureControl.InnerHandleService.response");
			}
			if (httpSessionState_0 == null)
			{
				throw new ArgumentNullException("WebTemperatureControl.InnerHandleService.session");
			}
			...
			if (a == "getpageimage")
			{
				...
				TemperatureDocument temperatureDocument = httpSessionState_0[text] as TemperatureDocument;
				...
				int result = 0;
				if (!string.IsNullOrEmpty(text) && int.TryParse(s, out result) && result >= 0 && result < temperatureDocument.NumOfPages)
				{
					MemoryStream memoryStream = new MemoryStream();
					using (Bitmap bitmap2 = temperatureDocument.CreatePageBmp(result))
					{
		...
		public bool LoadDocument(string strUrl, string format)
		{
			...
			if (Document == null)
			{
				Document = new TemperatureDocument();
			}
		...
		protected override void AddAttributesToRender(HtmlTextWriter writer)
		{
			...
			if (Page == null || Page.Site == null || !Page.Site.DesignMode)
			{
				...
				writer.AddAttribute("license", TemperatureDocument.LicenseTitle);
			}
		}
		...
		protected override void RenderContents(HtmlTextWriter writer)
		{
			...
			TemperatureDocument document = Document;
			...
			if (document.ViewMode == DocumentViewMode.Timeline)
			{
				using (Bitmap image = new Bitmap(1, 1))
				{
					using (Graphics graphics_ = Graphics.FromImage(image))
					{
						SizeF sizeF = document.method_6(graphics_);
			...
			for (int i = 0; i < document.NumOfPages; i++)
			{
				...
				writer.WriteLine(string.Concat("<div id=\"DCTimeLineimgcontent", num2, "\"  style=\"text-align:left;width:100%;overflow:auto;height:", Height, "\">"));
				writer.WriteLine("<img  class=\"DCTimeLinebigimg" + num2 + "\"  src=\"" + RuntimeServicePageURL + "?serviceflag=fdjia8324&sessionname=" + HttpUtility.HtmlAttributeEncode(RuntimeDocumentBufferedName) + "&method=getpageimage&pageindex=" + i + "&NoneFlag=" + Convert.ToString(int_2++) + "\" border='1' usemap='#" + text2 + "' ");
				writer.WriteLine("/>");
				...
				writer.WriteLine("</div>");
				document.dictionary_1 = new Dictionary<ValuePoint, RectangleF>();
				using (document.CreatePageBmp(i))
				{
				}
				if (document.dictionary_1.Count > 0)
				{
					writer.WriteLine("<map name='" + text2 + "'>");
					foreach (ValuePoint key in document.dictionary_1.Keys)
					{
						if (!string.IsNullOrEmpty(key.Link))
						{
							RectangleF rect = document.dictionary_1[key];
				...
				writer.WriteLine("function DCTimeLineselection(e) {");
				writer.WriteLine("var fobj = browser ? e.target : event.srcElement;");
				writer.WriteLine("if (fobj.className == \"DCTimeLinebigimg" + num2 + "\") {");
				...
				writer.WriteLine("document.onmousemove = DCTimeLinemovemouse;return false;}}");
				writer.WriteLine("function DCTimeLinemovemouse(e) {");
				...
				writer.WriteLine("}}");
				writer.WriteLine("document.getElementById(\"DCTimeLineimgcontent" + num2 + "\").onmousedown = DCTimeLineselection; document.getElementById(\"DCTimeLineimgcontent" + num2 + "\").onmouseup = new Function(\"isdrag=false\");");
