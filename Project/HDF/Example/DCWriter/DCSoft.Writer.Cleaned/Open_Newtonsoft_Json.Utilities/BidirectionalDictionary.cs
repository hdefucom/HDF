...
namespace Open_Newtonsoft_Json.Utilities
{
	...
	internal class BidirectionalDictionary<TFirst, TSecond>
	{
		...
		public void Set(TFirst first, TSecond second)
		{
			...
			{
				throw new ArgumentException(StringUtils.FormatWith(_duplicateFirstErrorMessage, CultureInfo.InvariantCulture, first));
			}
			if (_secondToFirst.TryGetValue(second, out TFirst value2) && !value2.Equals(first))
			{
				throw new ArgumentException(StringUtils.FormatWith(_duplicateSecondErrorMessage, CultureInfo.InvariantCulture, second));
			}
