...
namespace Open_Newtonsoft_Json.Utilities
{
	...
	internal struct DateTimeParser
	{
		...
		private bool Parse4Digit(int start, out int int_0)
		{
			if (start + 3 < _length)
			{
				int num = _text[start] - 48;
				int num2 = _text[start + 1] - 48;
				int num3 = _text[start + 2] - 48;
				int num4 = _text[start + 3] - 48;
				if (0 <= num && num < 10 && 0 <= num2 && num2 < 10 && 0 <= num3 && num3 < 10 && 0 <= num4 && num4 < 10)
				{
					int_0 = ((num * 10 + num2) * 10 + num3) * 10 + num4;
					return true;
				}
			}
			int_0 = 0;
		...
		private bool Parse2Digit(int start, out int int_0)
		{
			if (start + 1 < _length)
			{
				int num = _text[start] - 48;
		...
		private bool ParseChar(int start, char char_0)
		{
			return start < _length && _text[start] == char_0;
		}
	}
}
