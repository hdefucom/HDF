...
{
	                                                                    /// <summary>
	                                                                    ///       DBaseIII的DBF数据库文件的数据读取器
	                                                                    ///       </summary>
	[ComVisible(false)]
	public class DBaseIIIFileReader : DBaseFileReader
	{
		public DBaseIIIFileReader(string strFileName)
			: base(strFileName)
		{
		}

		public DBaseIIIFileReader(Stream stream)
			: base(stream)
		{
		}
		...
		protected override void ReadFileHeader()
		{
			...
			if (b != 3)
			{
				throw new FormatException(DataStrings.BadDBFFormat);
			}
			byte b2 = myReader.ReadByte();
			byte month = myReader.ReadByte();
			byte day = myReader.ReadByte();
			dtmLastUpdatedDate = new DateTime(b2 + 1900, month, day);
