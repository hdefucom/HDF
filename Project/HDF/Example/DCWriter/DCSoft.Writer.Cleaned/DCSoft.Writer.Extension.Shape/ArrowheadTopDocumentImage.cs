...
namespace DCSoft.Writer.Extension.Shape
{
	...
	///       指向上的箭头T
	///       </summary>
	[ToolboxBitmap(typeof(ArrowheadTopDocumentImage))]
	[ComVisible(false)]
	[DefaultProperty("Text")]
	public class ArrowheadTopDocumentImage : IDocumentImage
	{
		...
		private PointF[] GetPolygonPoints(RectangleF bounds)
		{
			...
			array[2].X = bounds.X + (bounds.Width - bounds.Width / 4f);
			array[2].Y = bounds.Y + bounds.Height / 2f;
			array[3].X = bounds.X + (bounds.Width - bounds.Width / 4f);
			array[3].Y = bounds.Y + bounds.Height;
			array[4].X = bounds.X + bounds.Width / 4f;
			array[4].Y = bounds.Y + bounds.Height;
			array[5].X = bounds.X + bounds.Width / 4f;
			array[5].Y = bounds.Y + bounds.Height / 2f;
			array[6].X = bounds.X;
			array[6].Y = bounds.Y + bounds.Height / 2f;
			array[7].X = bounds.X + bounds.Width / 2f;
			array[7].Y = bounds.Y;
			return array;
		}
