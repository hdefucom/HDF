...
namespace DCSoft.Writer.Extension.Shape
{
	...
	///       </summary>
	[ComVisible(false)]
	[ToolboxBitmap(typeof(ArrowheadLeftDocumentImage))]
	[DefaultProperty("Text")]
	public class ArrowheadLeftDocumentImage : IDocumentImage
	{
		...
		private PointF[] GetPolygonPoints(RectangleF bounds)
		{
			...
			array[0].X = bounds.X;
			array[0].Y = bounds.Y + bounds.Height / 2f;
			array[1].X = bounds.X + bounds.Width / 2f;
			array[1].Y = bounds.Y;
			array[2].X = bounds.X + bounds.Width / 2f;
			array[2].Y = bounds.Y + bounds.Height / 4f;
			array[3].X = bounds.X + bounds.Width;
			array[3].Y = bounds.Y + bounds.Height / 4f;
			array[4].X = bounds.X + bounds.Width;
			array[4].Y = bounds.Y + (bounds.Height - bounds.Height / 4f);
			array[5].X = bounds.X + bounds.Width / 2f;
			array[5].Y = bounds.Y + (bounds.Height - bounds.Height / 4f);
			array[6].X = bounds.X + bounds.Width / 2f;
			array[6].Y = bounds.Y + bounds.Height;
			array[7].X = bounds.X;
			array[7].Y = bounds.Y + bounds.Height / 2f;
			return array;
		}
		...
		public void Draw(DocumentPaintEventArgs args)
		{
			...
			if (!string.IsNullOrEmpty(Text))
			{
				using (DrawStringFormatExt drawStringFormatExt = new DrawStringFormatExt())
				{
					...
					args.Graphics.DrawString(Text, args.Style.Font, args.Style.Color, args.ClientViewBounds, drawStringFormatExt);
				}
			}
