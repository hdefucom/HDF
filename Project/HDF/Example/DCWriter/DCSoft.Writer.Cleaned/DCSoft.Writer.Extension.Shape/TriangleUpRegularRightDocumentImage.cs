...
namespace DCSoft.Writer.Extension.Shape
{
	...
	///       直角在右边的直角三角形
	///       </summary>
	[ToolboxBitmap(typeof(TriangleUpRegularRightDocumentImage))]
	[ComVisible(false)]
	[DefaultProperty("Text")]
	public class TriangleUpRegularRightDocumentImage : IDocumentImage
	{
		...
		private PointF[] GetPolygonPoints(RectangleF bounds)
		{
			...
			array[0].X = bounds.X + bounds.Width;
			array[0].Y = bounds.Y;
			array[1].X = bounds.X + bounds.Width;
			array[1].Y = bounds.Y + bounds.Height;
			array[2].X = bounds.X;
