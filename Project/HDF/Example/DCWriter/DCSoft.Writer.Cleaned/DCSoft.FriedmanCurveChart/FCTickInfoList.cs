...
namespace DCSoft.FriedmanCurveChart
{
	...
	///       </summary>
	[Serializable]
	[Editor(typeof(GClass153), typeof(UITypeEditor))]
	[DocumentComment]
	public class FCTickInfoList : List<FCTickInfo>
	{
		/// <summary>
		///       填充时刻成员
		///       </summary>
		...
		internal void FillTickItems(int tickStepSeconds, float totalSeconds, string specifyFormatString)
		{
			...
			for (int i = 0; (float)i < totalSeconds; i += tickStepSeconds)
			{
				...
				fCTickInfo.Text = text;
				fCTickInfo.Value = (float)i / 3600f;
				Add(fCTickInfo);
			}
		}
		...
		{
			Add(new FCTickInfo(int_0, string_0, color_0));
		}
		...
		public override string ToString()
		{
			...
			using (Enumerator enumerator = GetEnumerator())
			{
				...
				{
					FCTickInfo current = enumerator.Current;
		...
		///       </summary>
		/// <returns>复制品</returns>
		public FCTickInfoList Clone()
		{
			FCTickInfoList fCTickInfoList = new FCTickInfoList();
			using (Enumerator enumerator = GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					FCTickInfo current = enumerator.Current;
					fCTickInfoList.Add(current.Clone());
				}
			}
			return fCTickInfoList;
		}
	}
}
