...
namespace DCSoft.FriedmanCurveChart
{
	...
	///       </summary>
	[ComVisible(false)]
	public class FCdlgTickList : Form
	{
		private FCTickInfoList fctickInfoList_0 = null;
		...
		///       时刻列表
		///       </summary>
		public FCTickInfoList InputTicks
		{
			get
			{
				return fctickInfoList_0;
			}
			set
			{
				fctickInfoList_0 = value;
			}
		}
		...
		///       初始化对象
		///       </summary>
		public FCdlgTickList()
		{
			InitializeComponent();
			base.DialogResult = DialogResult.Cancel;
		}

		private void FCdlgTickList_Load(object sender, EventArgs e)
		{
			if (fctickInfoList_0 != null)
			{
				foreach (FCTickInfo item in fctickInfoList_0)
				{
		...
		private void btnSetTickBySeconds_Click(object sender, EventArgs e)
		{
			...
			if (num2 > 100 && MessageBox.Show(this, DCFriedmanCurveStrings.WarringSmallTickStep, Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.No)
			{
				return;
			}
			dgvTicks.Rows.Clear();
		...
		private void btnOK_Click(object sender, EventArgs e)
		{
			if (fctickInfoList_0 == null)
			{
				fctickInfoList_0 = new FCTickInfoList();
			}
			else
			{
				fctickInfoList_0.Clear();
			}
			foreach (DataGridViewRow item in (IEnumerable)dgvTicks.Rows)
			{
				if (!item.IsNewRow)
				{
					try
					{
						FCTickInfo fCTickInfo = new FCTickInfo();
						fCTickInfo.Text = Convert.ToString(item.Cells[0].Value);
						fCTickInfo.Value = Convert.ToSingle(item.Cells[1].Value);
						string text = Convert.ToString(item.Cells[2].Value);
						if (!string.IsNullOrEmpty(text))
						{
							fCTickInfo.Color = ColorTranslator.FromHtml(text);
						}
						fctickInfoList_0.Add(fCTickInfo);
					}
					catch (Exception ex)
					{
		...
		{
			System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(DCSoft.FriedmanCurveChart.FCdlgTickList));
			...
			base.Load += new System.EventHandler(FCdlgTickList_Load);
