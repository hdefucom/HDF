...
namespace DCSoft.Writer.Controls
{
	...
	internal class XTextInputFieldElementDateValueEditor : ElementValueEditor
	{
		...
		private sealed class Class280
		{
			...
			{
				int num = 17;
				if (class279_0.xtextInputFieldElement_0.InnerValue != string_0)
				{
					if (class279_0.textWindowsFormsEditorHost_0.Document.CanLogUndo)
					{
						class279_0.textWindowsFormsEditorHost_0.Document.UndoList.AddProperty("InnerValue", class279_0.xtextInputFieldElement_0.InnerValue, string_0, class279_0.xtextInputFieldElement_0);
					}
					class279_0.xtextInputFieldElement_0.InnerValue = string_0;
				}
				class279_0.xtextInputFieldElement_0.OwnerDocument.CopySourceExecuter.imethod_2(class279_0.xtextInputFieldElement_0);
			}
		}
		...
		public override ElementValueEditResult EditValue(TextWindowsFormsEditorHost host, ElementValueEditContext context)
		{
			...
			using (DateSelectControl dateSelectControl = new DateSelectControl())
			{
				dateSelectControl.DateTimeValue = result;
				dateSelectControl.EditorService = host;
				host.DropDownControl(dateSelectControl);
				flag = dateSelectControl.Modified;
				d = dateSelectControl.DateTimeValue;
			}
			if (flag)
			{
				...
				{
					text2 = ((xtextInputFieldElement_0.DisplayFormat == null || xtextInputFieldElement_0.DisplayFormat.IsEmpty) ? d.ToString("yyyy-MM-dd") : xtextInputFieldElement_0.DisplayFormat.Execute(d.ToString()));
					...
					{
						string_0 = xtextInputFieldElement_0.DisplayFormat.Execute(d.ToString());
					}
					else
					{
						string_0 = d.ToString("yyyy-MM-dd");
					}
				}
				if (xtextInputFieldElement_0.Text != text2)
				{
					...
					WriterBeforeFieldContentEditEventArgs writerBeforeFieldContentEditEventArgs = new WriterBeforeFieldContentEditEventArgs(xtextInputFieldElement_0, null, null, "XTextInputFieldElementDateValueEditor");
					...
					if (xtextInputFieldElement_0.SetEditorText(setContainerTextArgs))
					{
						...
						WriterAfterFieldContentEditEventArgs writerAfterFieldContentEditEventArgs_ = new WriterAfterFieldContentEditEventArgs(xtextInputFieldElement_0, null, null, "XTextInputFieldElementDateValueEditor", text3, innerValue);
						writerBeforeFieldContentEditEventArgs.NewText = text2;
						writerBeforeFieldContentEditEventArgs.NewValue = string_0;
